...
for i in eachindex(joint_initial_population)
    ...
    c4 = 1 + rand() * 1e-5
    c5 = 1 + rand() * 1e-5
    ground_truth_expr = parse_expression(:(($c1)*($c2*Main.SymbolicRegression.CoreModule.OperatorsModule.safe_pow(x1,4)+$c3*x1*Main.SymbolicRegression.CoreModule.OperatorsModule.safe_pow(x2,3)+$c4*x1*x2 + $c5*x2)), operators=joint_options.operators, variable_names=["x1", "x2"], expression_type=expr_type, node_type=node_type)
